...
```smart header="Квадратните скоби в синтаксиса `[...]`"
Квадратните скоби около `default` в синтаксиса по-горе означават, че параметърът е по необходимост, а не е задължителен.
```
...
````warn header="В браузъра Internet Explorer (IE): винаги предоставете `default`"
Вторият параметър е незадължителен, но ако не го предоставим, Internet Explorer ще вмъкне текста `"undefined"`.
...
alert( isBoss ); // true ако се натисне ОК бутона
```
