...
В тази статия ще научим повече за различните видове сравнения, как JavaScript ги прави, включително някои важни особености.

В края ще намерите добра рецепта, за да избегнете проблеми, свързани със странноста на JavaScript.
...
```js run
alert( 2 > 1 );  // true (вярно)
alert( 2 == 1 ); // false (невярно)
alert( 2 != 1 ); // true (вярно)
```
...
В горните примери сравнението `'Z' > 'A'` стига до резултат на първата стъпка, докато за низовете `"Glow"` и `"Glee"` ще са нужни повече стъпки, тъй като се сравняват символ-по-символ:

1. `G` е същото като `G`.
2. `l` е същото като `l`.
3. `o` е по-голямо от `e`. Спира тук. Първият низ е по-голям.

```smart header="Сравненията не са по речник, а по реда на Unicode кодиране!"
...
```js run
alert( null > 0 );  // (1) невярно
alert( null == 0 ); // (2) невярно
alert( null >= 0 ); // (3) *!*вярно*/!*
```
...
```js run
alert( undefined > 0 ); // невярно (1)
alert( undefined < 0 ); // невярно (2)
alert( undefined == 0 ); // невярно (3)
```
...
- Сравнения `(1)` и `(2)` връщат `false`, защото `undefined` се преобразува в `NaN` и `NaN`е специален числов тип, който връща `false` за всички сравнения.
- Проверката за равенство `(3)` връща `false`, защото `undefined` е равно само на `null`, `undefined`, и на никоя друга стойност.
