...
Има три логически оператора в JavaScript: `||` (ИЛИ), `&&` (И), `!` (НЕ / ОТРИЦАНИЕ), `??` (Нулев условен оператор). Тук ще обхващаме първите три, а операторът `??` е в следващата статия.
...
1. **Получаване на първата вярна стойност от списък на променливи или изрази.**
    ...
    Например, да имаме следните променливи: `firstName`, `lastName` и `nickName`, всички по избор (т.е. могат да бъдят `undefined` или `false`)
...
Въпреки че, вариантът с `&&` изглежда по-къса, `if` е по-очевидна и има тенденция да бъде малко по-четлива. Затова препоръчваме да използвате всяка конструкция за нейната цел: `if` ако искаме `if` и `&&` ако искаме AND оператора.
````
